...
export default function LensActivities({styles, onSave}) {
  ...
  if (active) return <View style={styles.lensPrompt}><Text style={styles.lensTitle}>{active[0]}</Text><Text style={styles.lensBody}>{active[1]}</Text><TextInput autoFocus placeholder="Write a few words…" style={styles.lensInput} onSubmitEditing={e => { onSave(e.nativeEvent.text, active[2]); setActive(null); }} returnKeyType="done"/><Pressable onPress={() => setActive(null)}><Text style={styles.lensClose}>Not right now</Text></Pressable></View>;
  return <View style={styles.lensGrid}>{activities.map(item => <Pressable key={item[0]} style={styles.lensCard} onPress={() => setActive(item)}><Text style={styles.lensTitle}>{item[0]}</Text><Text style={styles.lensBody}>{item[1]}</Text></Pressable>)}</View>;
}
