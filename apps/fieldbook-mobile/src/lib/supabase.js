...
export async function saveMoment({body, tag, programCode}) { const session = await ensureYouthSession(); return supabase.from('fieldbook_moments').insert({participant_id:session?.user?.id, body, tag, setting:programCode, private_to_participant:true, shared_with_program:false}); }
...
export async function loadYouthData() { const session = await ensureYouthSession(); if (!session?.user) return {mood:null, moments:[]}; const [moods,moments] = await Promise.all([supabase.from('fieldbook_mood_entries').select('x,y').eq('participant_id',session.user.id).order('created_at',{ascending:false}).limit(1), supabase.from('fieldbook_moments').select('body').eq('participant_id',session.user.id).order('created_at',{ascending:false}).limit(3)]); return {mood:moods.data?.[0] || null, moments:moments.data?.map(x=>x.body).filter(Boolean) || []}; }
