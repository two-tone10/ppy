...
export async function saveStaffSnapshot({touchpoint, observedText, tag, programCode}) { const {data:{user}}=await supabase.auth.getUser(); return supabase.from('fieldbook_staff_snapshots').insert({staff_id:user?.id, touchpoint, observed_text:observedText, tag, program_cycle:programCode||'pilot'}); }
export async function loadYouthData() { const session = await ensureYouthSession(); if (!session?.user) return {mood:null, moments:[], signals:[]}; const [moods,moments] = await Promise.all([supabase.from('fieldbook_mood_entries').select('x,y').eq('participant_id',session.user.id).order('created_at',{ascending:false}).limit(1), supabase.from('fieldbook_moments').select('body,tag,magnitude,created_at').eq('participant_id',session.user.id).order('created_at',{ascending:false}).limit(30)]); return {mood:moods.data?.[0] || null, moments:moments.data?.map(x=>x.body).filter(Boolean) || [], signals:moments.data?.map(x=>({body:x.body,tag:x.tag,magnitude:x.magnitude,created_at:x.created_at})).filter(x=>x.body) || []}; }
